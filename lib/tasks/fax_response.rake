...
task :check_fax_response => :environment do
  ...
  fax_requests_queue_ids.each do |fax_requests_queue_id|
    begin
      ...
      p "error requesting status for fax #{fax_requests_queue_id}"
...
task :sendback_final_response_to_client => :environment do
  ...
  if array_of_jsons.blank?
    ...
    uri = URI('https://mc1-efax-u.discoveryhealthpartners.com/eFaxService/OutboundDispositionService.svc/receive') #client URL
