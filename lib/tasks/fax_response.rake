...
desc "Resending Faxes with Busy number message and result code 6000"
  ...
  FaxRecord.send_error.each do |fax|
    ...
    if ( (fax[:resend]).between?(0,4) ) && ( (fax[:record_completed] == false) )  # DON'T FORGET TO CHANGE THE 100 TO 4
      ...
      begin
        ...
        fax.update_attributes(status: false,result_message: "Transmission not completed", result_code: '7001')
        Rails.logger.debug "==> Error resending_faxes_with_busy_line: #{fax.id} <=="
      end
    end
  end
end
