    ...
    class << self
      def file_path(file_key, fax_id)
        ...
        if Dir.exist?(files_folder)
          if file_key.size == (Dir["#{files_folder}/*"]).size
            ...
            audit_trails_attributes = {action: 'WebServices::Web.file_path', actor: Etc.getlogin, actor_type: 0, event: "Files attached #{Dir["#{files_folder}/*"]} to fax #{fax_id}", event_type:'info'}
            ...
            audit_trails_attributes = {action: 'WebServices::Web.file_path', actor: Etc.getlogin, actor_type: 0, event: 'Missing one or more attachments', event_type:'error'}
          ...
          if (Dir["#{files_dir}/*"]).empty?
            audit_trails_attributes = {action: 'WebServices::Web.file_path', actor: Etc.getlogin, actor_type: 0, event: "No files downloaded for keys #{file_key}", event_type:'error'}
            ...
            audit_trails_attributes = {action: 'WebServices::Web.file_path', actor: Etc.getlogin, actor_type: 0, event: "Files attached to fax #{Dir["#{files_dir}/*"]}", event_type:'info'}
      ...
      def client_fax_service_status(state)
        CallbackServer.all.each do |server|
          audit_trails_attributes = {action: 'client_fax_service_status', actor: Etc.getlogin, actor_type: 0, event: "Sending fax service status to #{server.name}: #{state}", event_type:'info'}
          ...
          begin
            ...
            audit_trails_attributes = {action: 'client_fax_service_status', actor: Etc.getlogin, actor_type: 0, event: "Fail to send fax service status to #{server.name}", event_type:'error'}
