class FaxRecord < ApplicationRecord
  ...
  scope :has_send_error,-> { where(result_code: '6000').where(result_message: 'Fax Number Busy').where(record_completed: false)}
  ...
  def self.to_csv(options = {})
    ...
    attributes = %w{id recipient_name recipient_number  message result_message attempts pages sender_fax created_at client_receipt_date send_confirm_date fax_duration}
